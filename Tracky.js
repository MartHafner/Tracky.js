...
function helper() {
    ...
    console.log(chalk.blue (' --Mail, -m <Mail>: Sucht nach E-Mail Adressen'));
    Zeilenumbruch(3);
...
function Mail(parameter) {
    const Mailsearch = child_process.spawn('node', ['Mail_Checker.js', String(parameter)],{
        env: {
            ...process.env,
            FORCE_COLOR: '1'
        }
    ...
    //Schreibt die Ergebnisse des Moduls in den Terminal
    Mailsearch.stdout.on('data', (data) => {
        process.stdout.write(data);
    });
    Mailsearch.stderr.on('data', (data) => {
        console.error(chalk.red(`[MODUL-FEHLER]: ${data}`));
    });
    Mailsearch.on('close', (code) => {
        console.log(chalk.yellow(`\nModul beendet mit Code: ${code}`));
        process.exit(0); // Programm wird ohne Fehler beendet
    });
}
...
async function Arghandler(args) {
    ...
    switch (LowercaseArgs) {
        ...
        case "-m":
            Zeilenumbruch(2)
            console.log(chalk.blue(`Die angegebene Mail ist: ${parameter}`));
            if (proxyArg === '--proxy' || proxyArg === '-p') {
                Mail(parameter, proxyURL)
            }
            else{
                Mail(parameter)
            }
